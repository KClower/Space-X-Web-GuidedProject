...
test("Re-renders correctly with list of missions", () => {
    ...
    // EXAMPLE of assert that there are no missions yet
    const missionsList = queryAllByTestId(/mission/i);
    expect(missionsList).toHaveLength(0);

    //  Act (re-render component with missions data passed in)
...
test("displays an error message if an error gets passed in", () => {
    // render the component with no error (be sure to pass in the missions prop)
    // assert that there is no error message displayed
    // re-render the component WITH an error passed down
    // assert that is now renders the error message
